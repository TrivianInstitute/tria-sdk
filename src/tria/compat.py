...
def require_supported_compatibility(
    event_schema_version: str,
    *,
    projection_version: str | None = None,
    bundle_format_version: str | None = None,
    ...
    if event_schema_version not in SUPPORTED_EVENT_SCHEMA_VERSIONS:
        failures.append(
            f"event schema {event_schema_version!r} (supported: {sorted(SUPPORTED_EVENT_SCHEMA_VERSIONS)!r})"
        )
    if projection_version is not None and projection_version not in SUPPORTED_PROJECTION_VERSIONS:
        failures.append(
            f"projection {projection_version!r} (supported: {sorted(SUPPORTED_PROJECTION_VERSIONS)!r})"
        )
    if bundle_format_version is not None and bundle_format_version not in SUPPORTED_BUNDLE_FORMAT_VERSIONS:
        failures.append(
            f"bundle format {bundle_format_version!r} (supported: {sorted(SUPPORTED_BUNDLE_FORMAT_VERSIONS)!r})"
        )
